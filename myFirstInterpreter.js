...
//! Исходя из описания выше и установленного мной опытным путем)) вот резюме этой запутанной задачи:
//! надо сделать интерпритатор языка в котором всего две команды '+' и '.':
...
//! В строке из плюсов и точек зашифрованы буквы/слова/фразы/символы
//! кол-во плюсов до точки соответствует номеру символа в Юникоде, в десятичном формате
//! в ячейке памяти умещается только 256 плюсов, поэтому входные данные надо будет разбить по 256.
//! все что не является точкой или плюсом не задействует память ячейки и должно отбрасываться при интерпритации этого кода
//! в ячейке памяти каждый следующий символ равен сумме плюсов предыдущих символов в этой ячейке и кол-ву плюсов до следующей точки (т.е. следующего символа)
//! Если десятичный номер символа не умещается в ячейке, то отчет плюсов продолжается в следующихячейках памяти.
//! если десятичный номер следующего символа меньше номера следующиго символа, то оставшаяся часть ячейки памяти заполняется плюсами (как баластом) чтобы отчет плюсов следующего символа начинался с новой ячейки памяти.
//! В конце берем набор номеров символов юникода и преобразовываем в буквы/слова/фразы/символы


function myFirstInterpreter(code) {
  if (!code.length || !/[\+].*[\.]/.test(code)) {
    return 0;
  }

  let charsDraft = code.match(/[\+\.]/g).join('').split('.').map(item => item.length);

  const chars = [charsDraft[0]];

  let usedMemory = charsDraft[0];

  for (let i = 1; i < charsDraft.length; i++) {
    usedMemory += charsDraft[i];
    ...
    if (usedMemory >= 256) {
      chars.push(usedMemory - 256);
      usedMemory = usedMemory % 256;
      continue;
    }

    chars.push(chars[chars.length - 1] + charsDraft[i])

  }

  // console.log(charsDraft);
  // console.log(chars);
...
console.log(myFirstInterpreter("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++.+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++."), "BA");
